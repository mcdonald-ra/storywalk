...
if($page<1){
	...
	$row=mysql_fetch_row($query);
	if($row){
		echo '<div id="bookwrap" style="position:relative;max-width:447px;max-height:673;display:block;margin-left:auto;margin-right:auto;">';
		echo '<div id="title" style="z-index:100; position:absolute; text-align:center; width:50%; top:15%; left:27%; color:white;">';
...
if($row){
	...
	if(!is_null($row[2])){
		echo '<img src="' . $row[2] . '" id="image1" alt="Image 1">';
	}
	
	echo '<div id="storytext">' . $row[0] . '</div>';
	
	if(!is_null($row[3])){
		echo '<img src="' . $row[3] . '" id="image2" alt="Image 2">';
	}
